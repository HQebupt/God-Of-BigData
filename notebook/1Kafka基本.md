...
<img src="1Kafka基本.assets/image-20210519153329698-1621409611342-1621516639096.png" alt="image-20210519153329698" style="zoom:50%;" />
...
| 导致适用场景不同 | 吞吐量高，适用数据实时收集 | 可靠性高，适用点对点数据传递，企业总线                       | 延时得看场景，kafka是异步批量，延迟偏大，RocketMQ写本地盘，理论上延迟更低 |
...
|                                        | RabbitMQ（erlang）                                        | RocketMQ                                              | Kafka                                                        | Pulsar                                                       |
| -------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 消息可靠性                             | 较好                                                      | 异步刷盘，同步刷盘，同步Replication，异步Replication         | 异步刷盘方式，异步Replication                                | 很高                                                         |
| 成熟度                                 | 成熟                                                      | 成熟（经历多个双十一）                                       | 高                                                           | 中                                                           |
| 单机吞吐量                             | 万                                                        | 10万+                                                        | 20万+                                                        | 100万 +                                                      |
| 特点                                   | 不支持延迟队列、消息过滤、消息回溯、消息保留。            | 主从HA；不支持优先级队列、非持久化主题、消息生存时间、多租户、多中心。 | 高吞吐量、持久化数据存储、分布式系统易于扩展，性能极好       | 灵活、多租户、云原生架构、跨地域复制；不支持事务消息、优先级队列、消息过滤 |
| 可用性                                 | 高（主从构架）                                            | 高（主从构架）                                               | 非常高（分布式）                                             | 非常高（分布式）                                             |
| 设计和架构方面                         | Push / Pull，AMQP协议，broker由Exchange,Binding,queue组成 | Push / Pull                                                  | pull方式消费消息，MQ协议                                     | 计算和存储分离（Broker和Segment分开存储）                    |
| 适用场景                               | 企业总线，可靠性高，适用点对点数据传递，                  | 企业总线，非日志的可靠消息传输，                             | 日志传输，追求吞吐量高                                       | 计算存储分离，云原生，延时比Kafka更低                        |
| 负载均衡                               | 无                                                        |                                                              | Zookeeper协调                                                | Zookeeper 和Bookeeper                                        |
| 顺序                                   |                                                           | 保证严格的消费顺序                                           | 支持消息顺序，但是一台Broker宕机后，就会产生消息乱序                  |                                                              |
| 单机支持的队列数（consumer集群的支持） |                                                           | 单机支持最高5万个队列，Load不会发生明显变化                  | Kafka单机超过64个队列/分区，Load会发生明显的飙高现象，队列越多，load越高，发送消息响应时间变长 |                                                              |
|消息推送时效性 ||长轮询，同Push方式实时性一致，消息的投递延时通常在几个毫秒。|短轮询方式，实时性取决于轮询间隔时间||
|消息查询机制||支持根据Message Id查询消息，也支持根据消息内容查询消息（发送消息时指定一个Message Key，任意字符串，例如指定为订单Id）|不支持消息查询||