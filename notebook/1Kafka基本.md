...
| ---------------- | -------------------------- | ------------------------------------------------------------ | -------------- |
| 设计和架构方面   | pull方式消费消息，MQ协议   | push方式消费消息，AMQP协议，broker由Exchange,Binding,queue组成 | 计算和存储分离 |
| 导致适用场景不同 | 吞吐量高，适用数据实时收集 | 可靠性高，适用点对点数据传递，企业总线                       |                |
| 负载均衡         | Zookeeper来协调            | 没有，需要自己实现                                           |                |
